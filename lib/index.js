...
module.exports = class Sempli {
    constructor(){
        ...
        this.vueConfig = {
            rootPath: path.join(path.resolve(),'src',this.views.directory),
    ...
    privateRoutes(directory,route){
       return this.express.use(directory,body,this.middlewareAuth,require(path.join(path.resolve(),'src/routes',route)));
    }
    ...
    generateTokens(user){
        ...
        return jwt.encode(payload,process.env['tokenSecret'] ? process.env['tokenSecret'] : this.auth['tokenSecret'])
    }
    ...
    middlewareAuth(req,res,next){
        ...
        let payload = jwt.decode(token,process.env['tokenSecret'] ? process.env['tokenSecret'] : this.auth['tokenSecret']);
